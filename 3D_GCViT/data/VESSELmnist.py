...
def compute_normalization(dataset):
    loader = DataLoader(dataset, batch_size=16, shuffle=False, num_workers=4, drop_last=True)
...
def get_dataloaders(batch_size=16, num_workers=4, prefetch_factor=2, pin_memory=True):
    """Loads the OrganMNIST3D dataset and applies computed normalization."""
    
    # --- Load datasets first (without transform) to compute mean & std ---
    train_dataset_raw = VesselMNIST3D(split='train', download=True)
    ...
    train_dataset = VesselMNIST3D(split='train', download=True, transform=transform)
    val_dataset = VesselMNIST3D(split='val', download=True,  transform=transform)
    test_dataset = VesselMNIST3D(split='test', download=True, transform=transform)
    ...
    train_loader_at_eval = DataLoader(train_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory, prefetch_factor=prefetch_factor, drop_last=False)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory, prefetch_factor=prefetch_factor, drop_last=False)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory, prefetch_factor=prefetch_factor, drop_last=False)
