...
# Custom transform to convert a 4D numpy array into a tensor
class ToTensor4D:
...
# Custom Dataset class to load the memory-mapped `.nii` file
class MemmapDataset(Dataset):
    def __init__(self, file_path, transform=None):
        # Create a memory-mapped array from the .nii file
        self.mmap_data = np.memmap(file_path, dtype='float32', mode='r', shape=(1, 128, 128, 128))  # Example shape (adjust as needed)
        self.transform = transform

    def __len__(self):
        # Assuming the first dimension is the number of samples
        return self.mmap_data.shape[0]

    def __getitem__(self, idx):
        sample = self.mmap_data[idx]
        if self.transform:
            sample = self.transform(sample)
        return sample, idx  # Example, you can add corresponding labels if you have them

def get_dataloaders(batch_size=16, num_workers=4, prefetch_factor=2, pin_memory=True):
    """Loads the dataset and applies computed normalization."""
    
    # --- Load datasets first (without transform) to compute mean & std ---
    file_path = "/home/wiebketeetz/ct_clip_data/train_1_a_1.nii"
    train_dataset_raw = MemmapDataset(file_path)
    ...
    train_dataset = MemmapDataset(file_path, transform=transform)
    val_dataset = train_dataset  # Assuming you want the same data for validation
    test_dataset = train_dataset  # Same here for testing
